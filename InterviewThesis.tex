...
In order to fulfill the goal of this paper, this paper will answer the following research questions:
\begin{enumerate}
...
\begin{algorithm}
	...
	\Output{Sorted List}
 	$n1 \longleftarrow $mid - left + 1\;
 	$n2 \longleftarrow $right - mid\;
 	$temp1[] \longleftarrow $[n1]\; 
 	$temp2[]\longleftarrow $[n2]\;
 	\tcc*[h]{Copy data into each temparray}
 	\For{$i\leftarrow 1$ \KwTo $n1$}{
		copy
	}
	\For{$i\leftarrow 1$ \KwTo $n2$}{
		copy
	}
	$i,j \longleftarrow $0\;
	$k \longleftarrow $left\;
	\While{$i < n1$ \&\& $j < n2$}{
	\If{$temp1[i] <= temp2[j]$ }{
		$A[k] = temp1[i]$\;
		$i++$\;
...
\begin{algorithm}
	...
	\Output{Sorted List}
 	\eIf{ left == right}{ return A[left]\;}
  	{$mid \longleftarrow $(left+right)/2\;
  	$ListNode $ \hlc[cyan!50]{l1}$ \leftarrow recMergeSort(A,left,mid)$\;
  	$ListNode $ \hlc[cyan!50]{l2}$ \leftarrow recMergeSort(A,mid+1,right)$\;
