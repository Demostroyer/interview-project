...
Whilst studying and working on this project, I have answered a number of questions from Leetcode.com which I was able to solve using an altered version of the above Palindrome pattern.\textbf{ID 1} TwoSum is one example. This question is described as: \textit{Given an array of integers, return indices of the two numbers such that they add up to a specific target. You may assume that each input would have exactly one solution, and you may not use the same element twice.} The following pseudocode is my answer to this question. In it I have taken the basic idea of the Palindrome algorithm of having two pointers used for looking through the input data, but I have altered the way these two pointers behave.In this pseudocode I have highlighted any differences in blue.
\IncMargin{1em}
\begin{algorithm}
	\SetAlgoLined
	\SetKwInOut{Input}{Input}
...
\begin{table}[t]
 ...
 Question Number & PassRate & Difficulty & Completion Time(mins) &Evaluation & Classical Algorithm \\
	 \hline
...
%\tablefirsthead{\toprule Question Number & PassRate & Difficulty & Time to Complete(mins) & My Evaluation & Classical Algorithm \\  \midrule }	
%
%\tablehead{%
%{{\bfseries Question Number & PassRate & Difficulty & Time to Complete(mins) & My Evaluation & Classical Algorithm  }} \\
%\toprule Question Number & PassRate & Difficulty & Time to Complete(mins) & My Evaluation & Classical Algorithm \\ \midrule}
%
%\tabletail{%
%\midrule Question Number & PassRate & Difficulty & Time to Complete(mins) & My Evaluation & Classical Algorithm \\  \midrule}
%\tablelasttail{%
%\\\midrule
%Question Number & PassRate & Difficulty & Time to Complete(mins) & My Evaluation & Classical Algorithm \\  \bottomrule}
...
\centering
	\begin{tabular}{| c | c | c |} \hline
		   & Achievable Invs & Unachievable Invs \\ \hline
		$(a)$ & $\{1,3,4,6,7,8\}$ & $\{2,8\}$ \\ \hline
		$(b)$ & $\{1,3,4,5,7,8\}$ & $\{2,6\}$ \\ \hline
		$(c)$ & $\{1,3,4,5,6,8\}$ & $\{2,7\}$ \\ \hline
		$(d)$ & $\{1,3,4,5,6,7\}$ & $\{2,8\}$ \\ \hline
		$(e)$ & $\{2,3,4,6,7,8\}$ & $\{1,5\}$ \\ \hline
		$(f)$ & $\{2,3,4,5,7,8\}$ & $\{1,6\}$ \\ \hline
		$(g)$ & $\{2,3,4,5,6,8\}$ & $\{1,7\}$ \\ \hline
		$(h)$ & $\{2,3,4,5,6,7\}$ & $\{1,8\}$ \\ \hline
	\end{tabular}
	\caption{A total of 8 possible ways of achieving a maximum number of 6 class invariants from the metamodel in Figure \ref{fig:running:example1}. The numbers $1$ - $8$ represent $8$ invariants in Figure \ref{fig:running:example1}.}
	\label{tab:running:example1}
\end{table}
\bibliography{mybib}
